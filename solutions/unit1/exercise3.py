...
def respond_to_weather(state: State) -> dict:
    ...
    if not isinstance(last_message, HumanMessage):
        raise ValueError("LLM did not return a human message")
    return {"messages": [ChatMessage(content=f"The weather is nice today. Last message: {last_message.content}")]}
...
def respond_to_not_weather(state: State) -> dict:
    ...
    if not isinstance(last_message, HumanMessage):
        raise ValueError("LLM did not return a human message")
    return {"messages": [ChatMessage(content=f"The weather is not nice today. Last message: {last_message.content}")]}
